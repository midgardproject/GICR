...
namespace GICR {
	...
	public interface Session : GLib.Object {
		 ...
		 */
		public const string ACTION_ADD_NODE = "add_node";
		 ...
		 *
		 * @return array containing the names of all attributes passed in the credentials used to acquire this session.
		 */
		public abstract string[] get_attribute_names ();

		/**
		 * Returns the value of the named attribute, or null if no
		 * attribute of the given name exists. See get_attribute_names ().
		 *
		 ...
		 *
		 * @return a {@link Workspace} object.
		 */
		 ...
		 *
		 * @param credentials {@link Credentials} object
		 ...
		 * application has this information, either get_node or get_property should be used, as appropriate. 
		 * In many repository implementations the node and property-specific methods are likely to be more
		 * efficient than get_item.
		 *
		 ...
		 *
		 * @param absPath An absolute path to an item to be found.
		 * @return boo True if the item exists, else false.
		 *
		 ...
		 * access to it; otherwise returns false.
		 *
		 * @param absPath An absolute path to the node to be found.
		 * @return True if the item does esist
		 *
		 * @throws RepositoryException if absPath is not a well-formed absolute path.
		 */
		 ...
		 * access to it; otherwise returns false.
		 *
		 * @param absPath An absolute path to the property to be found.
		 ...
		 * @return void
		 *
		 * @throws ItemExistsException if a node already exists at destAbsPath and same-name siblings are not allowed.
		 * @throws PathNotFoundException if either srcAbsPath or destAbsPath cannot be found and this implementation
		 *                                      performs this validation immediately.
		 * @throws VersionException if the parent node of destAbsPath or the parent node of srcAbsPath is
		 ...
		 * @throws LockException if the move operation would violate a lock and this implementation performs
		 *                                   this validation immediately.
		 ...
		 * @return void
		 *
		 * @throws VersionException if the parent node of the item at absPath is read-only due to a
		 ...
		 *                                             this validation immediately.
		 * @throws PathNotFoundException if no accessible item is found at absPath property or if the specified
		 ...
		 * @return void
		 *
		 * @throws AccessDeniedException if any of the changes to be persisted would violate the access privileges
		 ...
		 * @throws ItemExistsException if any of the changes to be persisted would be prevented by the presence
		 *                                    of an already existing item in the workspace.
		 * @throws ConstraintViolationException if any of the changes to be persisted would violate a node type or
		 ...
		 * @throws InvalidItemStateException if any of the changes to be persisted conflicts with a change already
		 ...
		 * @throws ReferentialIntegrityException if any of the changes to be persisted would cause the removal of a
		 *                                              node that is currently referenced by a REFERENCE property that
		 *                                              this Session has read access to.
		 * @throws VersionException if the save would make a result in a change to persistent storage
		 *                                         that would violate the read-only status of a checked-in node.
		 * @throws LockException if the save would result in a change to persistent storage that
		 *                                   would violate a lock.
		 * @throws NoSuchNodeTypeException if the save would result in the addition of a node with an
		 ...
		 * needed.
		 *
		 * @param keepChanges Switch to override current changes kept in the session.
		 * @return void
		 *
		 * @throws RepositoryException if an error occurs.
		 */
		 ...
		 * permitted, or throws a AccessControlException otherwise.
		 ...
		 * @param target FIXME the target object of the operation.
		 * @param arguments FIXME the arguments of the operation.
		 * @return false if the operation cannot be performed, true if the operation can be performed or if the
		 ...
		 *
		 * Returns an ContentHandler which is used to push SAX events
		 ...
		 * @return ContentHandler whose methods may be called to feed SAX events into the deserializer.
		 *
		 * @throws PathNotFoundException if no node exists at parentAbsPath and this implementation performs this
		 *                                      validation immediately.
		 * @throws ConstraintViolationException if the new subgraph cannot be added to the node at parentAbsPath
		 *                                             due to node-type or other implementation-specific constraints, and
		 *                                             this implementation performs this validation immediately.
		 * @throws VersionException if the node at parentAbsPath is read-only due to a checked-in node and
		 *                                         this implementation performs this validation immediately.
		 * @throws LockException if a lock prevents the addition of the subgraph and this implementation
		 *                                   performs this validation immediately.
		 * @throws RepositoryException if another error occurs.
		 */
		 ...
		 * Workspace.import_xml for a version
		 ...
		 * @throws PathNotFoundException if no node exists at parentAbsPath and this implementation performs this
		 *                                      validation immediately.
		 * @throws ItemExistsException if deserialization would overwrite an existing item and this implementation
		 *                                    performs this validation immediately.
		 * @throws ConstraintViolationException if a node type or other implementation-specific constraint is
		 ...
		 * @param stream The stream resource (i.e. aquired with fopen) to which the XML serialization of the subgraph will be output. Must support the fwrite method.
		 * @param skipBinary A boolean governing whether binary properties are to be serialized.
		 * @param noRecurse A boolean governing whether the subgraph at absPath is to be recursed.
		 * @return void
		 ...
		 * The output XML will be encoded in UTF-8.
		 *
		 * @param absPath The path of the root of the subgraph to be serialized. This must be the path to a node,
		 *                        not a property
		 * @param stream The stream resource (i.e. aquired with fopen) to which the XML serialization of the subgraph will be output. Must support the fwrite method.
		 * @param skipBinary A boolean governing whether binary properties are to be serialized.
		 * @param noRecurse A boolean governing whether the subgraph at absPath is to be recursed.
		 ...
		 * is added.
		 *
		 * @param prefix The namespace prefix to be set as identifier.
		 * @param uri The loaction of the namespace definition (usually a uri).
		 * @return void
		 *
		 * @throws NamespaceException if an attempt is made to map a namespace URI to a prefix beginning with
		 ...
		 *
		 * @return array of currently registered namespace prefixes.
		 *
		 * @throws RepositoryException if an error occurs
		 */
		 ...
		 * this Session.
		 *
		 * @param prefix The identifier of the namespace location to be returned.
		 * @return The location of the namespace definition identified by its prefix.
		 *
		 ...
		 *
		 * @param uri The loaction of the namespace definition (usually a uri).
		 * @return The prefix of a namespace identified by its uri.
		 *
		 ...
		 *
		 * @return AccessControlManager the access control manager for this Session
		 *
		 ...
		 *
		 * @return RetentionManager the retention manager for this Session.
		 *
