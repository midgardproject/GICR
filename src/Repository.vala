...
namespace GICR {
	 ...
	 * the value of this descriptor is the string "2.0".
    	 */
	const string SPEC_VERSION_DESC = "jcr.specification.version";
	 ...
	 */
	const string WRITE_SUPPORTED = "write.supported";
	 ...
	 *
	 */
	const string IDENTIFIER_STABILITY = "identifier.stability";
	 ...
	 */
	const string IDENTIFIER_STABILITY_METHOD_DURATION = "identifier.stability.method.duration";

	/**
	 * One of four possible values for the descriptor IDENTIFIER_STABILITY.
	 * Indicates that identifiers are guaranteed stable within a single save/refresh cycle.
	 */
	const string IDENTIFIER_STABILITY_SAVE_DURATION = "identifier.stability.save.duration";

	/**
	 * One of four possible values for the descriptor IDENTIFIER_STABILITY.
	 * Indicates that identifiers are guaranteed stable within a single session.
	 */
	const string IDENTIFIER_STABILITY_SESSION_DURATION = "identifier.stability.session.duration";

	/**
	 * One of four possible values for the descriptor IDENTIFIER_STABILITY.
	 * Indicates that identifiers are guaranteed to be stable forever.
	 */
	const string IDENTIFIER_STABILITY_INDEFINITE_DURATION = "identifier.stability.indefinite.duration";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if XML export is supported.
	 */
	const string OPTION_XML_EXPORT_SUPPORTED = "option.xml.export.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if XML import is supported.
	 */
	const string OPTION_XML_IMPORT_SUPPORTED = "option.xml.import.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if unfiled content is supported.
	 */
	const string OPTION_UNFILED_CONTENT_SUPPORTED = "option.unfiled.content.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if full versioning is supported.
	 */
	const string OPTION_VERSIONING_SUPPORTED = "option.versioning.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if simple versioning is supported.
	 */
	const string OPTION_SIMPLE_VERSIONING_SUPPORTED = "option.simple.versioning.supported";

	/**
	 * Key to a boolean descriptor. Returns true if and only if activities are
	 * supported.
	 */
	const string OPTION_ACTIVITIES_SUPPORTED = "option.activities.supported";

	/**
	 * Key to a boolean descriptor. Returns true if and only if configurations
	 * and baselines are supported.
	 */
	const string OPTION_BASELINES_SUPPORTED = "option.baselines.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if access control is supported.
	 */
	const string OPTION_ACCESS_CONTROL_SUPPORTED = "option.access.control.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if locking is supported.
	 */
	const string OPTION_LOCKING_SUPPORTED = "option.locking.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if asynchronous observation is supported.
	 */
	const string OPTION_OBSERVATION_SUPPORTED = "option.observation.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if journaled observation is supported.
	 */
	const string OPTION_JOURNALED_OBSERVATION_SUPPORTED = "option.journaled.observation.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if retention and hold are supported.
	 */
	const string OPTION_RETENTION_SUPPORTED = "option.retention.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if lifecycles are supported.
	 */
	const string OPTION_LIFECYCLE_SUPPORTED = "option.lifecycle.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if transactions are supported.
	 */
	const string OPTION_TRANSACTIONS_SUPPORTED = "option.transactions.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if workspace management is supported.
	 */
	const string OPTION_WORKSPACE_MANAGEMENT_SUPPORTED = "option.workspace.management.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if the primary node type of an existing node can be updated.
	 */
	const string OPTION_UPDATE_PRIMARY_NODETYPE_SUPPORTED = "option.update.primary.nodetype.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if the mixin node types of an existing node can be added and removed.
	 */
	const string OPTION_UPDATE_MIXIN_NODETYPES_SUPPORTED = "option.update.mixin.nodetypes.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if the creation of shareable nodes is supported.
	 */
	const string OPTION_SHAREABLE_NODES_SUPPORTED = "option.shareable.nodes.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if node type management is supported.
	 */
	const string OPTION_NODE_TYPE_MANAGEMENT_SUPPORTED = "option.node.type.management.supported";

	/**
	 * Key to a boolean descriptor. Returns true if and only if node and
	 * property with same name is supported.
	 */
	const string OPTION_NODE_AND_PROPERTY_WITH_SAME_NAME_SUPPORTED = "option.node.and.property.with.same.name.supported";
	 ...
	 */
	const string NODE_TYPE_MANAGEMENT_INHERITANCE = "node.type.management.inheritance";
	 ...
	 */
	const string NODE_TYPE_MANAGEMENT_INHERITANCE_MINIMAL = "node.type.management.inheritance.minimal";
	 ...
	 */
	const string NODE_TYPE_MANAGEMENT_INHERITANCE_SINGLE = "node.type.management.inheritance.single";
	 ...
	 */
	const string NODE_TYPE_MANAGEMENT_INHERITANCE_MULTIPLE = "node.type.management.inheritance.multiple";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if override of inherited property or child node definitions is supported.
	 */
	const string NODE_TYPE_MANAGEMENT_OVERRIDES_SUPPORTED = "node.type.management.overrides.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if primary items are supported.
	 */
	const string NODE_TYPE_MANAGEMENT_PRIMARY_ITEM_NAME_SUPPORTED = "node.type.management.primary.item.name.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if preservation of child node ordering is supported.
	 */
	const string NODE_TYPE_MANAGEMENT_ORDERABLE_CHILD_NODES_SUPPORTED = "node.type.management.orderable.child.nodes.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if residual property and child node definitions are supported.
	 */
	const string NODE_TYPE_MANAGEMENT_RESIDUAL_DEFINITIONS_SUPPORTED = "node.type.management.residual.definitions.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if autocreated properties and child nodes are supported.
	 */
	const string NODE_TYPE_MANAGEMENT_AUTOCREATED_DEFINITIONS_SUPPORTED = "node.type.management.autocreated.definitions.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if same-name sibling child nodes are supported.
	 */
	const string NODE_TYPE_MANAGEMENT_SAME_NAME_SIBLINGS_SUPPORTED = "node.type.management.same.name.siblings.supported";

	/**
	 * Key to an integer[] descriptor. Returns an array holding the
	 * GICR.PropertyType constants for the property types
	 ...
	 */
	const string NODE_TYPE_MANAGEMENT_PROPERTY_TYPES = "node.type.management.property.types";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if multivalue properties are supported.
	 */
	const string NODE_TYPE_MANAGEMENT_MULTIVALUED_PROPERTIES_SUPPORTED = "node.type.management.multivalued.properties.supported";
	 ...
	 */
	const string NODE_TYPE_MANAGEMENT_MULTIPLE_BINARY_PROPERTIES_SUPPORTED = "node.type.management.multiple.binary.properties.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only value-constraints are supported.
	 */
	const string NODE_TYPE_MANAGEMENT_VALUE_CONSTRAINTS_SUPPORTED = "node.type.management.value.constraints.supported";
	 ...
	 */
	const string QUERY_LANGUAGES = "query.languages";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if stored queries are supported.
	 */
	const string QUERY_STORED_QUERIES_SUPPORTED = "query.stored.queries.supported";

	/**
	 * Key to a boolean descriptor. Returns true
	 * if and only if full-text search is supported.
	 */
	const string QUERY_FULL_TEXT_SEARCH_SUPPORTED = "query.full.text.search.supported";
	 ...
	 *
	 */
	const string QUERY_JOINS = "query.joins";
	 ...
	 */
	const string QUERY_JOINS_NONE = "query.joins.none";

	/**
	 * One of three possible values for the descriptor QUERY_JOINS .
	 * Indicates that inner joins are supported.
	 */
	const string QUERY_JOINS_INNER = "query.joins.inner";

	/**
	 * One of three possible values for the descriptor QUERY_JOINS .
	 * Indicates that inner and outer joins are supported.
	 */
	const string QUERY_JOINS_INNER_OUTER = "query.joins.inner.outer";

	public errordomain RepositoryException {
		INTERNAL
	}

	public errordomain UnsupportedRepositoryOperationException {
		INTERNAL
	}

	public errordomain LoginException {
		INTERNAL
	}
	...
	public errordomain VersionException {
		VALUE_FORMAT		
	}
