...
namespace GICR {
	 ...
	 */
	const string NAME_WORKSPACE_ROOT = "";

	/**
	 * A constant for the absolute path of the workspace root node.
	 */
	const string PATH_WORKSPACE_ROOT = "/";

	/**
	 * A constant for the name of the system node.
	 */
	const string NAME_SYSTEM_NODE = "{http://www.jcp.org/jcr/1.0}system";

	/**
	 * A constant for the absolute path of the system node.
	 * This is '/' . NAME_SYSTEM_NODE
	 */
	const string PATH_SYSTEM_NODE = "{http://www.jcp.org/jcr/1.0}system";

	/**
	 * A constant for the name of the node type definition storage node.
	 */
	const string NAME_NODE_TYPES_NODE = "{http://www.jcp.org/jcr/1.0}nodeTypes";

	/**
	 * A constant for the absolute path of the node type definition storage node.
	 * This is PATH_SYSTEM_NODE . '/' . NAME_NODE_TYPES_NODE
	 */
	const string PATH_NODE_TYPES_NODE = "{http://www.jcp.org/jcr/1.0}system/{http://www.jcp.org/jcr/1.0}nodeTypes";

	/**
	 * A constant for the name of the version storage node.
	 */
	const string NAME_VERSION_STORAGE_NODE = "{http://www.jcp.org/jcr/1.0}versionStorage";

	/**
	 * A constant for the absolute path of the version storage node.
	 * This is PATH_SYSTEM_NODE . '/' . NAME_VERSION_STORAGE_NODE
	 */
	const string PATH_VERSION_STORAGE_NODE = "{http://www.jcp.org/jcr/1.0}system/{http://www.jcp.org/jcr/1.0}versionStorage";

	/**
	 * A constant for the name of the activities node.
	 */
	const string NAME_ACTIVITIES_NODE = "{http://www.jcp.org/jcr/1.0}activities";

	/**
	 * A constant for the absolute path of the activities node.
	 * This is PATH_SYSTEM_NODE . '/' . NAME_ACTIVITIES_NODE
	 */
	const string PATH_ACTIVITIES_NODE = "{http://www.jcp.org/jcr/1.0}system/{http://www.jcp.org/jcr/1.0}activities";

	/**
	 * A constant for the name of the configurations node.
	 */
	const string NAME_CONFIGURATIONS_NODE = "{http://www.jcp.org/jcr/1.0}configurations";

	/**
	 * A constant for the absolute path of the configurations node.
	 * This is PATH_SYSTEM_NODE . '/' . NAME_CONFIGURATIONS_NODE
	 */
	const string PATH_CONFIGURATIONS_NODE = "{http://www.jcp.org/jcr/1.0}system/{http://www.jcp.org/jcr/1.0}configurations";

	/**
	 * A constant for the name of the unfiled storage node.
	 */
	const string NAME_UNFILED_NODE = "{http://www.jcp.org/jcr/1.0}unfiled";

	/**
	 * A constant for the absolute path of the unfiled storage node.
	 * This is PATH_SYSTEM_NODE . '/' . NAME_UNFILED_NODE
	 */
	const string PATH_UNFILED_NODE = "{http://www.jcp.org/jcr/1.0}system/{http://www.jcp.org/jcr/1.0}unfiled";

	/**
	 * A constant for the name of the jcr:xmltext node produced on importXML().
	 */
	const string NAME_JCR_XMLTEXT = "{http://www.jcp.org/jcr/1.0}xmltext";

	/**
	 * A constant for the name of the jcr:xmlcharacters property produced on importXML().
	 */
	const string NAME_JCR_XMLCHARACTERS = "{http://www.jcp.org/jcr/1.0}xmlcharacters";
	 ...
	 */
	const string RELPATH_JCR_XMLCHARACTERS = "{http://www.jcp.org/jcr/1.0}xmltext/{http://www.jcp.org/jcr/1.0}xmlcharacters";
	 ...
	 * @return void
	 *
	 * @throws NoSuchWorkspaceException if srcWorkspace does not exist or if the current Session does not have permission to access it.
	 ...
	 * @throws VersionException if the parent node of destAbsPath is read-only due to a checked-in node.
	 * @throws AccessDeniedException if the current session does have access srcWorkspace but otherwise does not have sufficient access to complete the operation.
	 * @throws PathNotFoundException if the node at srcAbsPath in srcWorkspace or the parent of destAbsPath in this workspace does not exist.
	 * @throws ItemExistsException if a node already exists at destAbsPath and either same-name siblings are not allowed or update on copy is not supported for the nodes involved.
	 ...
	 * @throws ConstraintViolationException if the operation would violate a node-type or other
	 ...
	 * @throws PathNotFoundException if the node at srcAbsPath in srcWorkspace or the parent of destAbsPath in
	 *                                      this workspace does not exist.
	 * @throws ItemExistsException if a node already exists at destAbsPath and same-name siblings are not
	 ...
	 *                                             implementation-specific constraint
	 * @throws VersionException if the parent node of destAbsPath is read-only due to a checked-in node.
	 * @throws AccessDeniedException if the current session (i.e. the session that was used to acquire this
	 *                                      Workspace object) does not have sufficient access rights to complete the
	 *                                      operation.
	 * @throws PathNotFoundException if the node at srcAbsPath or the parent of destAbsPath does not exist.
	 ...
	 *
	 * @return an {@link ObservationManager} object.
	 ...
	 *
	 * @return VersionManager a VersionManager object.
	 ...
	 * @throws PathNotFoundException if no node exists at parentAbsPath.
	 * @throws ConstraintViolationException if node-type or other implementation-specific constraints prevent the addition of the subgraph or if uuidBehavior is set to IMPORT_UUID_COLLISION_REMOVE_EXISTING and an incoming node has the same identifier as the node at parentAbsPath or one of its ancestors.
	 * @throws VersionException if the node at parentAbsPath is read-only due to a checked-in node.
	 * @throws InvalidSerializedDataException if incoming stream is not a valid XML document.
	 * @throws ItemExistsException if the top-most element of the incoming XML would deserialize to a node with the same name as an existing child of parentAbsPath and that child does not allow same-name siblings, or if a uuidBehavior is set to IMPORT_UUID_COLLISION_THROW and an identifier collision occurs.
	 * @throws LockException if a lock prevents the addition of the subgraph.
	 ...
	 * implementations in optimizing subsequent Node.update () and
	 * Node.merge () calls between the new workspace and its source.
	 *
	 ...
	 * @return void
	 *
	 * @throws AccessDeniedException if the session through which this Workspace object was acquired does not have sufficient access to create the new workspace.
	 ...
	 * @return void
	 *
	 * @throws AccessDeniedException if the session through which this Workspace object was acquired does not have sufficient access to remove the workspace.
